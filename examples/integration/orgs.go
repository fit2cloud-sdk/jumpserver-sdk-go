...
func testOrgs() {
	...
	o, _, err := client.Orgs.Create(ctx, &model.OrganizationRequest{Name: "org-" + ts})
	if err != nil {
		fail("Organizations.Create", err)
	} else {
		ok("Organizations.Create (id=" + o.ID + ")")
		u, _, err := client.Orgs.Update(ctx, o.ID, &model.OrganizationRequest{Name: "org-upd-" + ts})
		if err != nil {
			fail("Organizations.Update", err)
		} else {
			ok("Organizations.Update (" + u.Name + ")")
		}
		got, _, err := client.Orgs.Get(ctx, o.ID)
		if err != nil {
			fail("Organizations.Get", err)
		} else {
			ok("Organizations.Get (" + got.Name + ")")
		}
		list, _, err := client.Orgs.List(ctx, &jumpserver.ListOptions{Limit: 15})
		if err != nil {
			fail("Organizations.List", err)
		} else {
			ok("Organizations.List (" + itoa(len(list)) + ")")
		}
		_, err = client.Orgs.Delete(ctx, o.ID)
		if err != nil {
